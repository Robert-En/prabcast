...
from setup_module.forecast_helpers import train_forecast_model, create_forecast_chart
...
def display_tab():
    ...
    **Univariate Absatzprognose:** Erstellen Sie Vorhersagen basierend auf historischen Verkaufsdaten 
    ...
    UI.page_header(
        ...
        subtitle="Univariate Zeitreihenprognose für ein Produkt",
    )

    # ✨ PROFESSIONELLER: Section Header
    UI.section_header(
        "Produkt und Zeitraum",
        help_text="Konfigurieren Sie die Grundeinstellungen für die Prognose",
    )
    ...
    with col2:
        ...
        with col2b:
            ...
            if st.button(
                "🤖",
                key="show_smart_recommendations",
                help="Zeige intelligente Empfehlungen",
            ):
                st.session_state["show_recommendations"] = not st.session_state.get(
        ...
        with col2c:
            ...
            if st.button(
                "📖", key="btn_horizon_help", help="Was ist ein Prognosehorizont?"
            ):
                st.session_state["popup_horizon_help"] = not st.session_state.get(
                    "popup_horizon_help", False
                )
    ...
    horizon_warning = HelpSystem.check_forecast_horizon(
        len(product_data), forecast_horizon
    )
    if horizon_warning:
    ...
    if st.session_state.get("show_recommendations", False):
        with st.expander("Intelligente Analyse & Empfehlungen", expanded=True):
            ...
            # Horizon
            st.success(
                f"**Empfohlener Prognosehorizont:** {recommended_horizon} Monate"
            )
    ...
    if st.button(
        ...
        key="model_selection_help",
        help="Hilfe zur Modellauswahl",
    ):
        st.session_state["show_model_help"] = not st.session_state.get(
            "show_model_help", False
        )
    ...
    selected_model, model_metadata = display_model_selector_with_info(
        ...
        default_models=recommendations["models"]["primary"][
            :1
        ],  # Beste Empfehlung vorauswählen
    )
    ...
    if UI.primary_button("Prognose erstellen", key="forecast_simple_run"):
        ...
        with display_spinner("Erstelle Prognose..."):
            ...
            success, result = safe_execute(
                ...
                fanchart_config=fanchart_config,
            )
            ...
            display_forecast_results(
                result, selected_product, forecast_horizon, fanchart_config
            )


def run_forecast_simple(
    state,
    selected_product,
    forecast_horizon,
    selected_model,
    fanchart_config=None,
) -> Dict[str, Any]:
    """
    ...
    # Modell trainieren
    model_params = {}
    if selected_model == "Prophet" and fanchart_config:
        model_params["interval_width"] = fanchart_config["ci_upper_bound"] / 100

    model = train_forecast_model(model_class, product_data, **model_params)
    if model is None:
...
def display_forecast_results(
    result, selected_product, forecast_horizon, fanchart_config: dict
) -> None:
    """
    ...
    UI.section_header(
        "Prognose-Ergebnisse", help_text="Visualisierung und Download der Prognosewerte"
    )
